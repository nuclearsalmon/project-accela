package net.accela.prisma.terminal.chars;
...
public enum C0ControlCode {
    ...
    private final String caretNotation;

    C0ControlCode(byte by) {
        this(by, null);
    }

    C0ControlCode(byte by, @Nullable String caretNotation) {
        // This ensures that there are no duplicate identifiers.
        for (C0ControlCode value : values()) {
            // Byte
            if (value.getByte() == by) {
                throw new IllegalStateException(String.format(
                        "Duplicate byte for %s and %s", value, this
                ));
            }

            // Caret notation
            String valueCaretNotation = value.getCaretNotation();
            if (valueCaretNotation != null && valueCaretNotation.equals(caretNotation)) {
                throw new IllegalStateException(String.format(
                        "Duplicate caret notation for %s and %s", value, this
                ));
            }
        }

        this.by = by;
        this.caretNotation = caretNotation == null ? null : caretNotation.intern();
    }
    ...
    public @Nullable String getCaretNotation() {
