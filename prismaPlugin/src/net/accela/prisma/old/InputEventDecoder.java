 ...
 * <p>
 * At most times it will return null, but if it does catch a sgr
 * then it will create an {@link InputEvent} from it and return that one.
 */
@Deprecated
public class InputEventDecoder {
    final Plugin plugin;
    ...
    public InputEventDecoder(@NotNull Plugin plugin) {
        this.plugin = plugin;
    }
     ...
     *  <esc> <nochar>                   -> esc
     *  <esc> <esc>                      -> esc
     *  <esc> <char>                     -> Alt-keypress or keycode sgr
     *  <esc> '[' <nochar>               -> Alt-[
     *  <esc> '[' (<num>) (';'<num>) '~' -> keycode sgr, <num> defaults to 1
     * </pre>
     */
    @Nullable
    public InputEvent parse(@NotNull final String singleEntry) {
        switch (parserState) {
            case GROUND:
                // ESC
                if (singleEntry.equals(ANSIUtils.ESC_STRING)) {
            ...
            case ANSI_IDENT:
                ...
                if (Character.isLetter(singleEntry.charAt(0)) || singleEntry.charAt(0) == '~') {
                    if (singleEntry.equals("M")) parserState = ParserState.ANSI_MOUSE;
                        ...
                        // Parse the sgr, reset and return
            ...
            case ANSI_MOUSE:
                ...
                // Check if the sgr is long enough to be complete
    ...
    MouseEvent parseMouse(@NotNull final String sequence) {
        // Netrunner spec
        if (sequence.charAt(1) == '0') {
            ...
            return new MouseEvent(plugin, x, y, MouseEvent.MouseInputType.LEFT);
        }
        // xterm spec, or at least assume so
            ...
            if (motion) {
                return new MouseEvent(plugin, x, y, MouseEvent.MouseInputType.MOTION, shift, meta, control);
            } else {
                MouseEvent.MouseInputType mouseInputType = intToMouseInputEvent(button);
                if (mouseInputType == null) return null;
                else return new MouseEvent(plugin, x, y, mouseInputType, shift, meta, control);
            }
        }
    }
    ...
    @Nullable
    MouseEvent.MouseInputType intToMouseInputEvent(int mouseButton) {
        switch (mouseButton) {
            case 0:
                return MouseEvent.MouseInputType.LEFT;
            case 1:
                return MouseEvent.MouseInputType.MIDDLE;
            case 2:
                return MouseEvent.MouseInputType.RIGHT;
            case 3:
                return MouseEvent.MouseInputType.RELEASE;
            case 4:
                return MouseEvent.MouseInputType.MOTION;
            case 5:
                return MouseEvent.MouseInputType.SCROLL_UP;
            case 6:
                return MouseEvent.MouseInputType.SCROLL_DOWN;
            case 7:
                return MouseEvent.MouseInputType.SCROLL_LEFT;
            case 8:
                return MouseEvent.MouseInputType.SCROLL_RIGHT;
            case 9:
                return MouseEvent.MouseInputType.EXTRA_0;
            case 10:
                return MouseEvent.MouseInputType.EXTRA_1;
            case 11:
                return MouseEvent.MouseInputType.EXTRA_2;
            case 12:
                return MouseEvent.MouseInputType.EXTRA_3;
            default:
                return null;
        }
    }
    ...
    InputEvent parseANSI(@NotNull final String sequence) {
        ...
        // If the modifier comes first ( for example <esc>[5C or <esc>[1;5C )
            ...
            // If it's a Point
            if (lastChar == 'R') {
                return new PointEvent(plugin, new Point(Integer.parseInt(keycode), Integer.parseInt(modifier)));
            }
            ...
            Logger.getLogger("InputEventDecoder").log(
