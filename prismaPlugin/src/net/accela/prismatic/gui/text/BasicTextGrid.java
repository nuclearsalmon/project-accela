package net.accela.prismatic.gui.text;
...
public class BasicTextGrid implements TextGrid {
    ...
    private @NotNull TextCharacter[][] buffer;
    ...
    public BasicTextGrid(@NotNull Size size) {
        this(size, TextCharacter.DEFAULT);
    }
    ...
    public void resize(@NotNull Size newSize, @NotNull TextCharacter filler) {
        ...
        synchronized (this) {
            BasicTextGrid result = getResize(newSize, filler);
    ...
    @Override
    @NotNull
    public BasicTextGrid getResize(@NotNull Size newSize, @NotNull TextCharacter filler) {
        if (newSize.getHeight() == buffer.length && newSize.getWidth() == buffer[0].length) {
            return this;
        }
