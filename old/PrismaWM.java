...
import net.accela.prisma.textgfx.deprecated.SGRSequence;
import net.accela.prisma.textgfx.deprecated.SGRStatement;
import net.accela.prisma.textgfx.deprecated.canvas.Canvas;
import net.accela.prisma.textgfx.deprecated.canvas.Cell;
import net.accela.prisma.annotation.Paintable;
 ...
 */
public class PrismaWM implements Paintable {
    /**
    ...
    // Mouse related
    // TODO: 10/23/20 move mouse handling to lowlevel
    ...
    public PrismaWM(@NotNull TextGraphicsSession session) {
        ...
        // Reset the lowlevel to its initial state and clear it
    ...
    public void attach(@NotNull Drawable drawable, @NotNull Plugin plugin)
        ...
        if (Main.DBG_RESPECT_TERMINAL_BOUNDS &&
            ...
            throw new RectOutOfBoundsException("Drawable does not fit within the lowlevel");
        }
            ...
            if (targetRect == null) {
                if (Main.DBG_RESPECT_TERMINAL_BOUNDS) {
                    throw new IllegalStateException(
                            "\n" + rect + "\n is outside the lowlevel boundaries \n" + termBounds);
                } else {
                    return;
                }
            }
            ...
            while (nodeIterator.hasPrevious()) {
                ...
                if (drawableCanvas.getSize().equals(drawableRect.getSize())) {
                    ...
                    String warnMsg = drawable.toString() + "\n"
                            + drawableCanvas + "\n"
                            + " tried to pass a TextBufferGrid with non-matching Size dimensions. The Drawable will be detached.";
                    throw new IllegalStateException(warnMsg);
                }
            }

            // Paint lowlevel from canvas
            for (int y = targetRect.getMinY(); y < targetRect.getMaxY() + 1; y++) {
                // Move the cursor into position
                writeToSession(CSIStatement.CUP(targetRect.getMinX() + 1, y + 1));
                ...
                for (int x = targetRect.getMinX(); x < targetRect.getMaxX() + 1; x++) {
                    ...
                    if (cell == null) {
                        // Reset painting attributes to prevent sgr bleed
                        /* fixme remove
                        terminalState.reset();
    ...
    /**
     * Moves the lowlevel cursor used for text editing.
    ...
    public void moveTextCursor(@NotNull Point point) {
        writeToSession(CSIStatement.CUP(point.getX() + 1, point.getY() + 1));
    }
     ...
     */
    public void callEvent(@NotNull PluginRequiredEvent event, @NotNull Drawable drawable) {
     ...
     */
    private void performEventBroadcast(@NotNull PluginRequiredEvent event) {
        broadcastLock.lock();
        try {
     ...
     * @param event The event to broadcast
     */
    void receiveEvent(final @NotNull PluginRequiredEvent event) {
        // Focus mods etc
    ...
    class BroadcastListener implements Listener {
        ...
        @EventHandler
        public void onMouseInputEvent(MouseEvent event) {
            receiveEvent(event);
        }

        @EventHandler
        public void onPointInputEvent(PointEvent event) {
            receiveEvent(event);
        }
